...
def create_extended_fe_dummy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    year_dummy = [col for col in df.columns if 'year_' in col]
    prefecture_dummy = [col for col in df.columns if 'prefecture_' in col]
    df = df[exog + ['actual_employment_rate', 'id', 'year']]
    df = df.set_index(['id', 'year'])
    
    formula = 'actual_employment_rate ~ ' + ' + '.join(exog) + \
            ' + ' + ' + '.join([f'total_regular_worker*{dummy}' for dummy in classcode_dummy]) + \
            ' + ' + ' + '.join([f'total_regular_worker*{dummy}' for dummy in year_dummy]) + \
            ' + ' + ' + '.join([f'total_regular_worker*{dummy}' for dummy in prefecture_dummy]) + ' + EntityEffects'
    result = PanelOLS.from_formula(formula, df, check_rank=False, drop_absorbed=True).fit()
    df['estimated_effects'] = result.estimated_effects
    return df
...
def main():
    ...
    # extended_feの作成
    df = pd.read_csv('datasets/dummy/dummy_extended.csv')
    df = create_extended_fe_dummy(df)
    df.to_csv('datasets/dummy/dummy_extended_fe.csv')
    ...
    # 固定効果を被説明変数とする分析
    df = pd.read_csv('datasets/dummy/dummy_extended_fe.csv')
